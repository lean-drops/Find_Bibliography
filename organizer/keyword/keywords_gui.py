"""
GUI (PySide6) – Konzept-Finder mit Auto-Session-Inkrement, Cache und HTML-Report.

Nutzung
-------
python organizer/keyword/keywords_gui.py

Funktionen
----------
Seed -> OpenAI-Kandidaten (mit Cache) -> Deduplizieren/Varianten -> Scan der Sekundärliteratur
-> JSON + HTML-Report (mit separater CSS/SCSS) im Session-Ordner. Nach Export wird der
Session-Name automatisch auf die nächste freie Nummer erhöht.
...
def _propose_initial_session(base_dir: Path) -> str:
    ...
    # Wenn vorhanden, an Nummer anhängen
    idx = 0
    while True:
...
def _next_session_name(current: str, base_dir: Path) -> str:
    """
    Erhöht numerischen Suffix. Falls none, fügt _01 an. Sucht freie Bezeichnung.
    """
...
class MainWindow(QMainWindow):
    ...
    def on_scan(self) -> None:
        ...
        patterns = keyword_utils.compile_family_patterns(
            [{"root": k, "variants": v} for k, v in self.expanded_terms.items()]
        )
        self.append_log(f"[DEBUG] Scan startet in {lib}")
        result = keyword_utils.scan_library_for_pattern(
            base_dir=lib,
            master_pattern=patterns,
            exclude_dir_name="chroniken",
            kwic_chars=int(self.kwic_spin.value()),
        )
        self.scan_result = result
        self.preview.setPlainText("Scan-Resultate:\n" + json.dumps(result["summary"], ensure_ascii=False, indent=2))
        self.append_log(f"[DEBUG] Scan fertig. Dateien={result['summary']['files_scanned']} Hits={result['summary']['total_hits']}")
    ...
    def on_export(self) -> None:
        # Session-Ordner eindeutig wählen
        ...
        # Master-Regex für Report sichern
        ...
        # Session-Name automatisch erhöhen
...
def main() -> None:
    print("[DEBUG] Start keywords_gui.py")
    app = QApplication([])
    apply_dark_theme(app)
    win = MainWindow()
    win.show()
    app.exec()
    print("[DEBUG] Ende keywords_gui.py")
